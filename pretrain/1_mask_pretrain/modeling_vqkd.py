...
import utils
from vqkd_teacher import clip, get_dino_vit_base, get_dino_vit_large
...
class VQKD(nn.Module):
    def __init__(self,
        ...
        freeze_encoder = encoder_config.pop('freeze_encoder', False)
    ...
    def get_regress_target(self, x, **kwargs):
        ...
        # Pathology teacher: UNI
        elif self.teacher_model_type == 'uni':
            target = self.teacher_model.forward_features(norm_imgs)[:, 1:, :] # [bsz, 196, 1024]
...
def vqkd_encoder_base_decoder_3x768x12_clip(pretrained=False, pretrained_weight=None, as_tokenzer=False, img_size=224, 
    ...
    if as_tokenzer:
        ...
        for k in keys:
            if k.startswith("loss") or k.startswith("teacher") or k.startswith("scaling") or k.startswith("decode_task_layer") or k.startswith("decoder"):
                del weights[k]
        
        # Interpolate position embedding
        if model.encoder.pos_embed.shape != weights['encoder.pos_embed'].shape:
            
            pos_embed = weights['encoder.pos_embed']
            new_token_len = model.encoder.pos_embed.shape[1]  # 577

            # Extract the CLS token
            cls_token = pos_embed[:, :1, :]

            # Extract the position embeddings (excluding the CLS token)
            pos_tokens = pos_embed[:, 1:, :]

            # Determine the original and new grid size (excluding CLS token)
            original_grid_size = int((pos_tokens.shape[1]) ** 0.5)  # sqrt(196) = 14
            new_grid_size = int(((new_token_len - 1) ** 0.5))  # sqrt(576) = 24

            # Reshape the position embeddings to match the grid shape
            pos_tokens = pos_tokens.reshape(1, original_grid_size, original_grid_size, -1).permute(0, 3, 1, 2)  # (1, 768, 14, 14)

            # Interpolate the position embeddings to the new grid size
            pos_tokens = F.interpolate(pos_tokens, size=(new_grid_size, new_grid_size), mode='bicubic', align_corners=False)

            # Reshape the interpolated position embeddings back to the original shape
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).reshape(1, -1, 768)

            # Concatenate the CLS token back with the interpolated position embeddings
            new_pos_embed = torch.cat((cls_token, pos_tokens), dim=1)

            weights['encoder.pos_embed'] = new_pos_embed
            
        missing, unexpected = model.load_state_dict(weights, strict=False)
        assert any(['encoder' not in item for item in missing])  # as long as no encoder modules are missing.
...
def vqkd_encoder_base_decoder_3x768x12_ctrans(pretrained=False, freeze_encoder=False, pretrained_weight=None, as_tokenzer=False, img_size=224, 
    ...
    encoder_config['img_size'] = img_size
    encoder_config['num_classes'] = 0
    encoder_config['freeze_encoder'] = freeze_encoder
    # decoder settings
    decoder_config['img_size'] = img_size // decoder_config['patch_size']
    decoder_config['patch_size'] = 1
    decoder_config['in_chans'] = code_dim
