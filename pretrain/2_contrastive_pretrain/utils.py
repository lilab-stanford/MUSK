...
def not_whole_word_mask(tokenizer, input_ids, mask_probability=0.15):
    ...
    if loss_scaler is not None:
        ...
        if args.auto_resume:
            ...
            if latest_ckpt >= 0:
                ...
                if args.start_epoch == 0:  # overwrite default setting
    ...
    for pos_embed_key in ("vision_pos_embed", "pos_embed", "beit3.encoder.embed_positions.A.weight"):
        if pos_embed_key in checkpoint_model:
            ...
            if pos_embed_key == "beit3.encoder.embed_positions.A.weight":
                ...
                # num_extra_tokens = model.beit3.vision_embed.num_position_embeddings() + 2 - num_patches - 31  # if we add 31 registers, use this.
            else:
...
def mm_collate_fn(batch):
    ...
    dict_batch = {k: [dic[k] if k in dic else None for dic in batch] for k in keys}  # the value of dict is list
    new_dict_batch = dict()

    for k in dict_batch.keys():
        values = dict_batch[k]
        values = [value.unsqueeze(0) if torch.is_tensor(value) else torch.tensor(value).unsqueeze(0)
                  for value in values]
        new_dict_batch[k] = torch.cat(values, dim=0)
...
# The implementation code is modified from https://github.com/ahmdtaha/distributed_sigmoid_loss/blob/main/distributed_sigmoid_loss.py 
class DDPSigmoidLoss(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.t_prime = nn.Parameter(torch.tensor(np.log(10.0), dtype=torch.float32))  # log 10
        self.bias = nn.Parameter(torch.tensor(-10.0, dtype=torch.float32))
        self.sigmoid_loss = nn.BCEWithLogitsLoss(reduction='sum')  # Use BCEWithLogitsLoss for numerical stability
    ...
    def forward(self, image_embeddings, text_embeddings):
        ...
        gpu_batch_size = image_embeddings.shape[0]
        ...
        def compute_device_loss(zimg, ztxt, same_device):
            ...
            if same_device:
                # Create target labels with diagonal 1 for matching pairs and 0 for non-matching pairs
                labels = torch.eye(gpu_batch_size, device=zimg.device)
            else:
                # Create target labels with all 0s for non-matching pairs
                labels = torch.zeros(gpu_batch_size, gpu_batch_size, device=zimg.device)

            loss = self.sigmoid_loss(logits, labels)
            return loss

        # Gather all text embeddings from all devices
        all_text_embeddings = [torch.zeros_like(text_embeddings) for _ in range(dist.get_world_size())]
        ...
        for i in range(world_size):
            gpu_batch_loss = compute_device_loss(
                image_embeddings, all_text_embeddings[i * gpu_batch_size:(i + 1) * gpu_batch_size], same_device=i == rank
            )
            total_loss += gpu_batch_loss

        total_loss = total_loss / (gpu_batch_size * world_size)
        return total_loss
...
def dump_predictions(args, result, file_suffix):
    ...
    if global_rank >= 0:
        ...
        if global_rank == 0:
            ...
            new_jsons = []
            res_dict = dict()
            if args.task in ["coco_captioning", "nocaps", "quilt1m_captioning"]:
                qid_key = "image_id"
            else:
                # for VQAv2
                qid_key = "question_id"
...
def pathology_caption_eval(reference_captions, candidate_caption, tokenizer):
    score = []
    new_candidate_caption = dict()
    candidate_caption = json.load(open(candidate_caption))
    for element in candidate_caption:
        new_candidate_caption[element['image_id']] = element['caption']

    for idx, item in enumerate(reference_captions):
        ref = tokenizer.decode(item['text_segment'])
        image_id = item['image_id']

        # len(new_candidate_caption) < len(reference_captions) because sometimes, one image_id has multiple captions
        if image_id not in new_candidate_caption:
            continue

        cap = new_candidate_caption[image_id]
        meteor = calculate_meteor(cap, ref)
        score.append(meteor)

    print(f"evaluate total image: {len(score)}")

    return {"meteor": sum(score)/len(score)}
