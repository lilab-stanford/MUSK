...
from dataset.datasets import create_downstream_dataset
...
def get_args():
    parser = argparse.ArgumentParser('Image-textcontrastive learning and evaluation script', add_help=False)

    # Model parameters
    parser.add_argument('--model', default='musk_base_patch16_224', type=str, metavar='MODEL',
                        help='Name of model to train')
    parser.add_argument('--task', type=str, required=True,
                        choices=['quilt1m_pathcap'],
    ...
    # parameter for mask decoder retrieval finetuning
    parser.add_argument('--text_mask_prob', type=float, default=0.15)
    # image dataset for joint training with mim
    parser.add_argument('--image_tsv', type=str, default=None, nargs="+")
...
def init_dist(launcher="slurm", backend='nccl', port=29500, **kwargs):
    ...
    if launcher == 'pytorch':
        ...
        utils.setup_for_distributed(utils.is_main_process())
...
def main(args, ds_init):
    ...
    args.gpu = init_dist(launcher='pytorch')
    ...
    if not args.eval:
        data_loader_train, data_loader_val = create_downstream_dataset(args, args.eval)
    else:
        data_loader_val = create_downstream_dataset(args, args.eval)
    ...
    print(
        f'number of trainable params: {n_parameters / 1024 / 1024:.3f}M of total {total_parameters / 1024 / 1024:.3f}M')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_grad, args.update_freq, model_ema, log_writer, args.task, None
        )
        ...
        if args.output_dir and args.save_ckpt:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                ...
                if utils.is_main_process():
                    torch.save(model_without_ddp.state_dict(), f"{args.output_dir}/epoch-{epoch}.pth")
